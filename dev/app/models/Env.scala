...
case class Bundle(bundleName:String,description:String,uId:String)

case class Exe(exeName:String,description:String,uId:String)
...
class EnvModel @Inject()(protected val dbConfigProvider: DatabaseConfigProvider){
  ...
  class BundleTable(tag: Tag) extends Table[Bundle](tag, "BUNDLE") {
    ...
    def * = (bundleName, description,uId) <> (Bundle.tupled, Bundle.unapply)
  }
  ...
  class ExeTable(tag: Tag) extends Table[Exe](tag, "EXE") {
    ...
    def * = (exeName, description,uId) <> (Exe.tupled, Exe.unapply)
  }
...
//    def retrieve(bundleName: String):Seq[Bundle]= {
//        val dbRetrieve =db.run(compiledCheckByBundleName(bundleName).result) 
//        val result = Await.result(dbRetrieve, Duration.Inf)
//        return result
//    }
...
//    }
    ...
    def checkByBundleName(bundleName:Rep[String]) = Bundles.filter{ bundle => bundle.bundleName like bundleName } 
    def compiledCheckByBundleName = Compiled(checkByBundleName _)
    
    def checkByBundleDescription(description:Rep[String]) = Bundles.filter{ bundle => bundle.description like description } 
    def compiledCheckByBundleDescription = Compiled(checkByBundleDescription _)

    def checkByBundleId(uId:Rep[String]) = Bundles.filter{ bundle => bundle.uId like uId } 
    ...
    def exeCheck(limit:ConstColumn[Long], max:ConstColumn[Long]) =  Exes.sortBy(_.exeName).drop(limit).take((max)) 
    def compiledExe = Compiled(exeCheck _)
    
    def checkByExeName(exeName:Rep[String]) = Exes.filter{ exe => exe.exeName like exeName } 
    def compiledCheckByExeName = Compiled(checkByExeName _)
    
    def checkByExeDescription(description:Rep[String]) = Exes.filter{ exe => exe.description like description } 
    def compiledCheckByExeDescription = Compiled(checkByExeDescription _)

    def checkByExeId(uId:Rep[String]) = Exes.filter{ exe => exe.uId like uId } 
